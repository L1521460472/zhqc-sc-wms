...
export default {
  methods: {
    ...
    queryListData(data) {
      const query = {
        spuCode: data.spu_code || this.spuCode
      }
      const _this = this
      this.$showLoading()
      this.$store.dispatch(this.store + 'pageInfo', query).then(() => {
        _this.initTopFormColumns()// 初始化查询界面配置数据
      }).finally(() => {
        this.$hideLoading()
      })
    },
    ...
    delete(data) {
      this.$confirm(this.$t('comm.deleteMsg'), {
        ...
        this.$store.dispatch(this.store + 'deleteData', { 'id': data.skuCode }).then(() => {
    ...
    editSave() {
      ...
      const { _id, ...resp } = this.diaFormInfo.data
      this.$store.dispatch(this.store + 'editData', resp).then(() => {
