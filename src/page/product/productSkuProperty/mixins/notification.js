...
export default {
  methods: {
    ...
    add() {
      ...
      this.diaFormInfo.subTableInfo.addShow = true
      this.diaFormInfo.subTableInfo.handle.btList[0].show = true
      this.diaFormInfo.subTableInfo.handle.btList[1].show = true
    ...
    // 添加子数据
    addSub() {
      this.diaFormInfo.subTableInfo.formInfo.data = {}
      this.diaFormInfo.subTableInfo.formInfo.visible = true
      this.diaFormInfo.subTableInfo.formInfo.btList[1].event = 'saveSub'
    },
    closeSub() {
      this.diaFormInfo.subTableInfo.formInfo.data = {}
      this.diaFormInfo.subTableInfo.formInfo.visible = false
    },
    saveSub() {
      this.diaFormInfo.subTableInfo.formInfo.ref.validate(validSon => {
        if (validSon) {
          this.diaFormInfo.data.subRecords.push(this.$deepClone(this.diaFormInfo.subTableInfo.formInfo.data))
          this.diaFormInfo.subTableInfo.formInfo.data = {}
          this.diaFormInfo.subTableInfo.formInfo.visible = false
        }
      })
    ...
    updateSub(data) {
      this.diaFormInfo.subTableInfo.formInfo.data = data
      this.diaFormInfo.subTableInfo.formInfo.visible = true
      this.diaFormInfo.subTableInfo.formInfo.btList[1].event = 'editSubSave'
    },
    deleteSub(data) {
      this.$confirm(this.$t('comm.deleteMsg'), {
        ...
        const tempArray = []
        this.diaFormInfo.data.subRecords.forEach((item) => {
          if (item.id !== data.id) {
            tempArray.push(item)
          }
        })
        this.diaFormInfo.data.subRecords = tempArray
      })
    },
    // 编辑按钮点击事件
    update(data) {
    ...
    view(data) {
      ...
      this.diaFormInfo.subTableInfo.addShow = false
      this.diaFormInfo.subTableInfo.handle.btList[0].show = false
      this.diaFormInfo.subTableInfo.handle.btList[1].show = false
    ...
    saveAdd() {
      ...
      this.diaFormInfo.ref.validate(valid => {
        if (valid) {
          that.$store.dispatch(that.store + 'saveData', that.diaFormInfo.data).then(() => {
            const resp = that.$store.state[that.modName].addResp
            if (resp.code === that.$store.state[that.modName].successCode) {
              that.dialogInfo.visible = false
              that.findPageList()
            }
          })
        }
