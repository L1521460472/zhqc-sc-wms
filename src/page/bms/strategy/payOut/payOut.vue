<template>
  <layout-body>
    ...
    <div slot="tab-body" class="tab-body_auto">
      ...
      >
        <template v-slot:bt-slotEvent="scope">
          ...
          <el-button v-if="scope.data.row.dispatchStatus === 4" type="danger" @click="disenable(scope.data.row)">{{ $t('payOut.disenable') }}</el-button>
          <el-button v-else type="primary" @click="enable(scope.data.row)">{{ $t('payOut.enable') }}</el-button>
        </template>
      </vex-table-fh>
    </div>
    ...
    <full-pop
      ...
      @handleClick="handleClick"
    >
      <full-pop-item :full-pop-item-title="$t('payOut.payOutInfo')">
        <!-- 主页面top表单 -->
        <zhqc-top-form-sc
      ...
      <full-pop-item :full-pop-item-title="$t('payOut.payOutDataRource')">
        <vex-dia-table
      ...
      <full-pop-item :full-pop-item-title="$t('payOut.billTemplate')">
        <zhqc-transfer :disabled="fullDialogInfo.type === 'view'" />
      </full-pop-item>
    </full-pop>
  </layout-body>
</template>
