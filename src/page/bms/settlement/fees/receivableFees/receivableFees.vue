<template>
  <div>
    <layout-body>
      ...
      <div slot="left-btn">
        <el-button-group>
          <el-button type="primary" icon="el-icon-folder-add" @click="handleClick('addFees')">{{ $t('receivableFees.addFees') }}</el-button>
          <el-button type="primary" @click="handleClick('calculate')">{{ $t('receivableFees.calculate') }}</el-button>
          <el-button type="primary" @click="handleClick('batchCalculate')">{{ $t('receivableFees.batchCalculate') }}</el-button>
          <el-button type="primary" @click="handleClick('payOut')">{{ $t('receivableFees.payOut') }}</el-button>
          <el-button type="primary" @click="handleClick('batchPayOut')">{{ $t('receivableFees.batchPayOut') }}</el-button>
          ...
          <template #1>
            ...
            >
              <template v-slot:bt-slotEvent="scope">
                <el-button type="success" @click="adjustmentFees(scope.data.row)">{{ $t('receivableFees.adjustmentFees') }}</el-button>
              </template>
            </zhqc-table>
          </template>
          <template #2>
    ...
    <receivable-fees-detail :visible="detailVisible" :list-type-info="listTypeInfo" />
...
import ReceivableFeesDetail from './components/receivableFeesDetail.vue'
import AddFees from './components/addFees.vue'
export default {
  name: 'ReceivableFees',
  components: {
    ReceivableFeesDetail,
    AddFees,
  ...
  data() {
    return {
      ...
      batchBtnArray: [
        { label: this.$t('receivableFees.review'), type: 'success', icon: '', event: 'review', btLoading: false, show: true },
        { label: this.$t('receivableFees.reviewAll'), type: 'success', icon: '', event: 'reviewAll', btLoading: false, show: true },
        { label: this.$t('receivableFees.reverseReview'), type: 'warning', icon: '', event: 'reverseReview', btLoading: false, show: true },
        { label: this.$t('receivableFees.reverseReviewAll'), type: 'warning', icon: '', event: 'reverseReviewAll', btLoading: false, show: true }
      ],
      tabs: {
        activeName: '1',
        ...
        fontSize: 14,
        tabsList: [
          { label: this.$t('receivableFees.all'), value: '1' },
          { label: this.$t('receivableFees.failCalculated'), value: '2' },
          { label: this.$t('receivableFees.reviewing'), value: '3' },
          { label: this.$t('receivableFees.reviewed'), value: '4' },
          { label: this.$t('receivableFees.billed'), value: '5' }
        ]
      },
