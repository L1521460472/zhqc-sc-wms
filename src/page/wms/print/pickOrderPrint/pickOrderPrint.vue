 ...
 * @LastEditTime: 2021-11-02 16:15:45
-->
<template>
  <layout-body>
    <div slot="top-form">
    ...
    <div slot="left-btn">
      <el-button-group>
        <!-- <el-button icon="el-icon-folder-add"  @click="handleClick('printOther')">新打印</el-button>-->
    ...
    <!--      -->
    <zhqc-dialog
      :title="dialogInfoP.title"
      :visible.sync="dialogInfoP.visible"
      :width="dialogInfoP.width"
      :bt-list="dialogInfoP.btList"
      @handleClick="handleClick"
    >
      <div ref="printxzse" class="jjnkxeer cen" v-html="daying" />
    </zhqc-dialog>
  </layout-body>
</template>
...
import pickOrderPrintMixins from './mixins'

export default {
  name: 'PickOrderPrint',
  mixins: [pickOrderPrintMixins],
  data() {
    return {
      store: 'pickOrderPrint/',
      modName: 'pickOrderPrint',
      ...
      dialogInfoP: {
        ...
        btList: [
          { label: this.$t('table.close'), type: '', icon: '', event: 'close', show: true },
          { label: '打印', type: 'primary', icon: '', event: 'printOtherSave', btLoading: false, show: true }]
      },
      daying: '',
      carrierCode: null
    }
