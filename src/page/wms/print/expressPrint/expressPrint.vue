 ...
 * @LastEditTime: 2021-11-02 16:15:13
-->
<template>
  <layout-body>
    <div slot="top-form">
      ...
      >
        ...
        <template v-slot:form-cusOrderNo="scope">
          <zhqc-area-input :model="topForm.data" order-no-key="cusOrderNo" @input="getCusOrderNo" />
        </template>
        <template v-slot:form-ownerId="scope">
          <remote-list
            :model="topForm.data"
    ...
    <div slot="left-btn">
      ...
      <el-button-group>
        <el-button type="warning" icon="el-icon-printer" :disabled="$hasPerm('expressOrder')" @click="handleClick('expressOrder')">下单打印</el-button>
      </el-button-group>

      <el-button-group>
        <el-button type="warning" :disabled="printList.length === 0" @click="handleClick('cancelOrder')">取消</el-button>
      </el-button-group>
    </div>
...
import expressPrintMixins from './mixins'

export default {
  name: 'ExpressPrint',
  mixins: [expressPrintMixins],
  data() {
    return {
      store: 'expressPrint/',
      modName: 'expressPrint',
  ...
  computed: {
    resp() {
      ...
      // return [{ waybillNo: 121212, cusOrderNo: 3242342 }]
    },
