<template>
  <layout-body>
    <div slot="top-form">
      ...
      >
        ...
        <template v-slot:form-lotCode="scope">
          <list-wh-lot :select-key="topForm.data.lotCode" @select="selectWhLot" />
        </template>
    ...
    <div slot="left-btn">
      ...
      <export-vue
        template-name="maintainProReportService"
        :export-url="exportUrl"
        export-name="养护记录报表导出"
...
import maintainProReportMixins from './mixins'
...
export default {
  name: 'MaintainProReport',
  components: {
    listSkuCode, listOwner, listWhLot, listWhZone
  },
  mixins: [maintainProReportMixins],
  data() {
    return {
      store: 'maintainProReport/',
      modName: 'maintainProReport',
      collapsable: false, // 展开收缩
      exportUrl: VUE_APP_WMS_MODEL + '/rpt/maintain/maintainProReport/export',
