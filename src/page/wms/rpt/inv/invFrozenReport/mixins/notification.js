...
export default {
  methods: {
    ...
    reboot() {
      ...
      this.topForm.data.frozenType = null
      this.topForm.data.sourceType = null
      this.topForm.data.frozenStatus = null
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          this.listTypeInfo.frozenTypeList = resp.obj.frozenTypeList
          this.listTypeInfo.frozenStatusList = resp.obj.frozenStatusList
