    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          this.listTypeInfo.paStatusList = resp.obj.paStatusList
          this.listTypeInfo.paTypeList = resp.obj.paTypeList
        }
      })
    },
    /**
