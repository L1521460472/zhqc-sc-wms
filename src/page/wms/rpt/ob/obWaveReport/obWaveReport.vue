 ...
 * @LastEditTime: 2021-11-02 17:05:28
-->
<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
  <layout-body>
    <div slot="top-form">
      ...
      >
        ...
        <template v-slot:form-customerId="scope">
          ...
          >
            <!-- <template v-slot="scope"> -->
            <span style="float: left; color: #8492a6; font-size: 13px;">{{ scope.item.customerName }}</span>
            <!-- </template> -->
          </remote-list>
        </template>
        <!-- 快递商 -->
        <template v-slot:form-partnerId="scope">
          ...
          >
            <!-- <template v-slot="scope"> -->
            <span style="float: left; color: #8492a6; font-size: 13px;">{{ scope.item.fullName }}</span>
            <!-- </template> -->
          </remote-list>
        </template>
        <!-- 产品编码 -->
    ...
    <div slot="left-btn">
      ...
      <export-vue
        template-name="obWaveReportService"
        :export-url="exportUrl"
        export-name="波次明细报表导出"
...
import obWaveReportMixins from './mixins'

export default {
  name: 'ObWaveReport',
  mixins: [obWaveReportMixins],
  data() {
    return {
      store: 'obWaveReport/',
      modName: 'obWaveReport',
      ...
      exportUrl: VUE_APP_WMS_MODEL + '/rpt/ob/obWaveReport/export',
