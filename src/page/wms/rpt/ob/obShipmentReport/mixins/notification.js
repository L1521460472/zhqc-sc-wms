...
export default {
  methods: {
    ...
    openViewPage(data) {
       ...
       */
      this.dialogInfo.title = this.$t('查看路由')
      // etLogisticsRoute  {"shippingOrder":"70221948839330"}   data.transOrderNo
      const reqData = {}
      reqData.shippingOrder = data.transOrderNo
      reqData.orderCode = data.customerOrderNo
      this.$store.dispatch(this.store + 'etLogisticsRoute', reqData).then(() => {
        this.dialogInfo.visible = true
      })
    },
    /**
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          this.listTypeInfo.scSoTypeList = resp.obj.scSoTypeList
