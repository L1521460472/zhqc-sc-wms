import { pageInfo, initPage, initUpdate, saveData, editData, deleteData, etLogisticsRoute } from './../api'
import { message } from '../../../../../../utils/messageUtils'
const obShipmentReport = {
  namespaced: true,
  state: {
    total: 0,
    successCode: 200,
  ...
  mutations: {
    ...
    ET_LOG_ISTICS_ROUTE(state, data) {
      if (data.code === 200) {
        state.routeResp = data.obj
      } else {
        state.routeResp = []
      }
    }
  ...
  actions: {
    ...
    //
    async etLogisticsRoute({ commit }, obj) {
      const resp = await etLogisticsRoute(obj)
      commit('ET_LOG_ISTICS_ROUTE', resp)
    }

  }
}
export default obShipmentReport
