...
export default {
  methods: {
    ...
    reboot() {
      ...
      this.topForm.data.recheckInfoDate = null
      ...
      this.topForm.data.recheckInfoDateFrom = null
      this.topForm.data.recheckInfoDateTo = null
      this.topForm.data.recheckUserNo = null
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          this.listTypeInfo.recheckInfoStatusList = resp.obj.recheckInfoStatusList
        }
      })
    },
    /**
