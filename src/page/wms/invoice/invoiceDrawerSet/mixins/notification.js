    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          ...
          this.listTypeInfo.invoiceVersionList = resp.obj.invoiceVersionList
          this.invoiceVersionMap = this.objConvMap(resp.obj.invoiceVersionMap)
        }
      })
    },
    /**
