  ...
  methods: {
    ...
    queryRowData(data) {
      this.$store.dispatch(this.store + 'initUpdate', data.id).then(() => {
        ...
        this.$set(this.diaFormInfo.data, 'openTicketQuota', this.invoiceVersionMap.get(this.diaFormInfo.data.invoiceVersion))
    ...
    batchDrawer() {
      ...
      this.$confirm(this.$t('确认批量开票?'), {
        type: 'warning',
        center: true
      }).then(() => {
        const idList = []
        for (let i = 0; i < kaiPiaoList.length; i++) {
          idList.push(kaiPiaoList[i].id)
        }
        this.$store.dispatch(this.store + 'drawer', idList).then(() => {
          const resp = this.$store.state[this.modName].drawerResp
          if (resp.code === this.$successCode) {
            this.queryTableData(this.topForm.data)
          }
        })
