...
export default {
  methods: {
    ...
    reboot() {
      ...
      this.topForm.data.carrierNickname = null
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          this.listTypeInfo.whTypeList = resp.obj.whTypeList
     ...
     */
    provinceList() {
      this.$store.dispatch(this.store + 'provinceList', {}).then(() => {
        const resp = this.$store.state[this.modName].provinceListResp
        if (resp.code === this.$store.state[this.modName].successCode) {
          this.listTypeInfo.provinceList = resp.obj
        }
      })
    },
