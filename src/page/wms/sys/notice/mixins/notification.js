...
export default {
  methods: {
     ...
     */
    reboot() {
      this.topForm.data.topic = null
      this.topForm.data.creator = null
      this.topForm.data.publisher = null
      this.topForm.data.noticeStatus = null
      this.topForm.data.publishTimeBegin = null
      this.topForm.data.publishTimeEnd = null
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        this.listTypeInfo.noticeStatusList = this.$store.state[this.modName].initPageObj.noticeStatusList
      })
    },
    /**
     * 查询表格数据
    ...
    queryRowData(data) {
      this.$store.dispatch(this.store + 'initUpdate', data.id).then(() => {
        ...
        // 模拟
        this.uploadInfo.fileList = this.diaFormInfo.data.resources
    ...
    saveData() {
      this.diaFormInfo.ref.validate(valid => {
        if (valid) {
          if (!this.diaFormInfo.data.htmlContent) {
            this.$message.error('请输入公告内容')
            return
          }
          this.$showLoading()
          const resources = this.uploadInfo.fileList.map((item) => {
            return {
              name: item.name,
              url: item.url
            }
    ...
    handelPublish(data) {
      this.$confirm(this.$t('确定要发布当前公告吗？'), {
        type: 'warning',
        center: true
      }).then(() => {
        this.$store.dispatch(this.store + 'publish', { 'id': data.id }).then(() => {
          const resp = this.$store.state[this.modName].publishResp
          if (resp.code === this.$successCode) {
            this.queryTableData(this.topForm.data)
          }
        })
    ...
    handelCancelPublish(data) {
      this.$confirm(this.$t('确定要取消发布当前公告吗？'), {
        type: 'warning',
        center: true
      }).then(() => {
        this.$store.dispatch(this.store + 'cancelPublish', { 'id': data.id }).then(() => {
          const resp = this.$store.state[this.modName].cancelPublishResp
          if (resp.code === this.$successCode) {
            this.queryTableData(this.topForm.data)
          }
        })
