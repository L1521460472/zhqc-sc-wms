  ...
  methods: {
    ...
    async validateFun(url) {
      ...
      if (errMap) {
        ...
        if (url === 'editData') {
          this.$confirm(this.$t('确认修改当前库存周转策略么？'), {
            type: 'warning',
            center: true
          }).then(() => {
          ...
          this.$confirm(this.$t('确认新增当前库存周转策略么？'), {
            type: 'warning',
            center: true
          }).then(() => {
            this.$showLoading()
    ...
    //
    addSub() {
      const subRecord = { id: new Date().getTime() }
      this.diaFormInfo.data.turnoverRuleDtPOList.push(subRecord)
    },
    //
    subDelete(val) {
      this.diaFormInfo.data.turnoverRuleDtPOList.splice(val.$rowIndex, 1)
    }
