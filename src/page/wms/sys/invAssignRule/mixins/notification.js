...
export default {
  methods: {
     ...
     */
    reboot() {
      this.topForm.data.ruleName = null
      this.topForm.data.ruleCode = null
      this.topForm.data.assignMethod = null
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        this.listTypeInfo.assignMethodList = obj.assignMethodList
        this.listTypeInfo.assignRuleList = obj.assignRuleList.map(item => {
          item.key = item.msg
    ...
    // 分配方式修改
    handleAssignMethod() {
      this.diaFormInfo.data.assignRule = null
      if (!this.diaFormInfo.data.assignMethod) {
        this.listTypeInfo.ruleList = []
        return
      }
      this.initAssignRule()
    },
    ...
    initAssignRule() {
      const array = []
      if (this.diaFormInfo.data.assignMethod == 'BATCH') {
        this.listTypeInfo.assignRuleList.forEach(item => {
          if (item.isBatch) {
            array.push(item)
          }
        })
      } else if (this.diaFormInfo.data.assignMethod == 'GOODS') {
        this.listTypeInfo.assignRuleList.forEach(item => {
          if (item.isGoods) {
            array.push(item)
          }
      ...
      this.listTypeInfo.ruleList = array
    },
    /**
     * 启用
     * @param data
