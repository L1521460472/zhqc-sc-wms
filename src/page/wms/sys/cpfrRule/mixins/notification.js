...
export default {
  methods: {
     ...
     */
    reboot() {
      this.topForm.data.cpfrRuleCode = null
      this.topForm.data.cpfrRuleName = null
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        const paRuleList = this.$store.state[this.modName].initPageObj.paRuleList.map((item) => {
          item.key = item.paRuleName
          item.value = item.id
          return item
        })
        this.listTypeInfo.paRuleList = paRuleList
      })
    },
    /**
     * 查询表格数据
