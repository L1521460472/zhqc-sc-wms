    ...
    <div slot="tab-body" class="tab-body_auto">
      ...
      >
        <template v-slot:bt-slotEvent="scope">
          <el-button v-if="scope.data.row.isSystem==0 " size="mini" type="primary" :disabled="$hasPerm('edit')" @click="openEditPage(scope.data.row)">编辑</el-button>
        </template>
      </zhqc-table>
    </div>
...
<script>
import invBatchAttrMixins from './mixins'
export default {
  name: 'InvBatchAttr',
  mixins: [invBatchAttrMixins],
  data() {
    return {
      store: 'invBatchAttr/',
      modName: 'invBatchAttr',
      formType: null,
      viewFlag: null,
