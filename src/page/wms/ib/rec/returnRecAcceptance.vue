  ...
  >
    ...
    <div slot="left-wrap">
      <el-form ref="form" :model="form" :rules="rules" label-position="top" size="small ">
        ...
        <div class="warehousing-section l-warehousing-flex">
          <div class="warehousing-section-left">
            <el-form-item label="产品编码" prop="skuNo">
              <el-input
                ref="skuNo"
                v-model="skuNo"
                :disabled="skuNoDis"
          ...
          <div class="warehousing-section-right l-warehousing-flex1 l-warehousing-flex">
            <div class="l-warehousing-flex1 l-warehousing-mr-10">
              <el-form-item label="店铺">
                <el-input v-model="shopCode" :disabled="true" />
              </el-form-item>
            </div>
            <div class="l-warehousing-flex1 l-warehousing-mr-10">
              <el-form-item label="所属平台">
                <el-input v-model="origPlatform" :disabled="true" />
              </el-form-item>
            </div>
            <div class="l-warehousing-flex1">
              <el-form-item label="退货原因">
                <el-input v-model="refundReason" :disabled="true" />
              </el-form-item>
            </div>
          </div>
        </div>
          ...
          <div class="warehousing-section-right l-warehousing-flex1 l-warehousing-flex">
            <div class="l-warehousing-flex1 l-warehousing-mr-10">
              <el-form-item label="退款类型">
                <el-input v-model="refundType" :disabled="true" />
              </el-form-item>
            </div>
            <div class="l-warehousing-flex1 l-warehousing-mr-10">
              <el-form-item label="源单号">
                <el-input v-model="sourceCode" :disabled="true" />
              </el-form-item>
            </div>
            <div class="l-warehousing-flex1">
              <el-form-item label="源批号">
                <el-input v-model="sourceBatchNo" :disabled="true" />
              </el-form-item>
            </div>
          </div>
        </div>
        ...
        <!--<el-input v-model="productionBatch"></el-input>-->
        <!--</el-form-item>-->
        <!--</div>-->
        <!--<div class="l-warehousing-flex1">-->
        <!--<el-form-item label="生产日期" prop="productionDate">-->
        <!--<el-date-picker v-model="productionDate" type="date" @change="querySkuInvalidDate"-->
        <!--placeholder="选择日期"></el-date-picker>-->
        <!--</el-form-item>-->
        <!--</div>-->
        <!--<div class="l-warehousing-flex1">-->
        <!--<el-form-item label="入库日期">-->
        <!--<el-input v-model="instoreDate" :disabled="true"></el-input>-->
        <!--</el-form-item>-->

        <!--</div>-->
        <!--<div class="l-warehousing-flex1">-->
        <!--<el-form-item label="有效期至">-->
        <!--<el-input v-model="invalidDate" :disabled="true"></el-input>-->
        <!--</el-form-item>-->
        <!--</div>-->
        <!--</div>-->
      </el-form>
    ...
    <div slot="ft-btn">
      <div class="ft-item-btn">
        <el-button type="primary" plain @click.native="nextSku">下一个SKU</el-button>
      </div>
      <div class="ft-item-btn">
        <el-button type="primary" @click.native="saveRecord">保存记录</el-button>
      </div>
      <div class="ft-item-btn">
        <el-button type="primary" plain @click.native="nextAsn">下一ASN</el-button>
      </div>
    </div>
  </full-by-pop>
</template>
...
export default {
  ...
  computed: {
    visible() {
      if (this.$store.state[this.modName].returnRecAcceptanceResp.visible) {
        // 初始化数据
        this.returnRecAcceptanceInitPage()
      }
      return this.$store.state[this.modName].returnRecAcceptanceResp.visible
    }
  },
  methods: {

    returnRecAcceptanceInitPage() {
    ...
    close() {
      ...
      this.clearSku()
      this.msg = null
      this.$store.dispatch(this.store + 'setData', { page: 'returnRecAcceptanceResp', visible: false })
    },
    ...
    scannOrderNo() {
      ...
      if (this.isEmpty(this.orderNo) || this.isClone) {
        return
      }
      this.isScan = true
      this.$store.dispatch(this.store + 'scannReturnOrderNo', this.orderNo).then(() => {
        const resp = this.$store.state[this.modName].scannReturnOrderNoResp
    ...
    /**
             ...
             */
    scannAreaNo() {
      this.$store.dispatch(this.store + 'scannReturnAreaNo', this.areaNo).then(() => {
        const resp = this.$store.state[this.modName].scannReturnAreaNoResp
        if (resp.code === this.$store.state[this.modName].successCode) {
    ...
    /**
             ...
             */
    scannContainerNo() {
      this.$store.dispatch(this.store + 'scannReturnContainerNo', this.containerNo).then(() => {
        const resp = this.$store.state[this.modName].scannReturnContainerNoResp
        if (resp.code === this.$store.state[this.modName].successCode) {
    ...
    scannSkuNo() {
      ...
      this.$store.dispatch(this.store + 'scannReturnSkuNo', this.skuData).then(() => {
        const resp = this.$store.state[this.modName].scannReturnSkuNoResp
        if (resp.code === this.$store.state[this.modName].successCode) {
          ...
          this.refundType = resp.obj.refundType
          this.shopCode = resp.obj.shopCode
          this.origPlatform = resp.obj.origPlatform
          this.refundReason = resp.obj.refundReason
          this.sourceCode = resp.obj.sourceCode
          this.sourceBatchNo = resp.obj.sourceBatchNo
    ...
    querySkuInvalidDate() {
      ...
      this.$store.dispatch(this.store + 'queryReturnSkuInvalidDate', this.skuInvalidData).then(() => {
        const resp = this.$store.state[this.modName].queryReturnSkuInvalidDateResp
        if (resp.code === this.$store.state[this.modName].successCode) {
    ...
    saveRecord() {
      ...
      this.diaFormInfoAttr.ref.validate(valid => {
        if (valid) {
          ...
          this.$store.dispatch(this.store + 'saveReturnRecInfo', this.saveRecordObj).then(() => {
            const resp = this.$store.state[this.modName].saveReturnRecordResp
            if (resp.code === this.$store.state[this.modName].successCode) {
              // 清空原有数据
              this.clearSku()
