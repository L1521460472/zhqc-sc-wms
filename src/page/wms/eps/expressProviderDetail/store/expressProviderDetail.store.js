import { pageInfo, initPage, initUpdate, saveData, editData, deleteData, synEpsProvider } from './../api'
import { message } from '../../../../../utils/messageUtils'
const expressProviderDetail = {
  namespaced: true,
  state: {
    total: 0,
    successCode: 200,
  ...
  mutations: {
    ...
    SYN_EPS_PROVIDER(state, data) {
      state.synEpsProviderResp = data
    }
  },
  actions: {
    // 查询事件
    ...
    // 同步快递服务商面单信息
    async synEpsProvider({ commit }, obj) {
      const resp = await synEpsProvider(obj)
      commit('SYN_EPS_PROVIDER', resp)
    }
  }
}
export default expressProviderDetail
