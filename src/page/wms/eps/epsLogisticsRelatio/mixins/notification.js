...
export default {
  methods: {
    ...
    /**
         ...
         */
    reboot() {
      this.topForm.data.platformType = null
      this.topForm.data.wmsLpCode = null
      this.topForm.data.lpCode = null
      this.topForm.data.lpId = null
    },
    /**
         * 翻页
         * @param val
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          this.listTypeInfo.epsPlatformTypeList = resp.obj.epsPlatformTypeList
        }
      })
    },
    /**
    ...
    queryRowData(data) {
      this.$store.dispatch(this.store + 'initUpdate', data.id).then(() => {
        ...
        this.diaFormInfo.data = resp.entity
        this.listTypeInfo.standardTemplateList = resp.standardTemplateList
        this.listTypeInfo.customizeTemplateList = resp.customizeTemplateList
        this.listTypeInfo.wmsLpCodeList = resp.wmsLpCodeList
      })
    },
    /**
         * 新增数据
