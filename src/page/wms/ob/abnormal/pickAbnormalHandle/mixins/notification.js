...
export default {
  methods: {
     ...
     * @param data
     */
    openAbnormalHandlePage(data) {
      // 弹窗的类型：edit
      this.dialogInfo.type = 'abnormalHandle'
      // 默认弹窗的标题：修改
      ...
      this.dialogInfo.btList[1].show = true
      // 绑定弹窗保存事件
      this.dialogInfo.btList[1].event = 'abnormalHandle'
      // 设置宽度
      this.dialogInfo.width = '800px'
      // 封装的修改表单的数据化配置
      this.diaFormInfoAbnormalHandleFieldList()
      // 为弹窗表单对应的字段赋值
      this.diaFormInfo.data = this.$deepClone(data)
    },
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          this.listTypeInfo.statusList = resp.obj.statusList
          this.listTypeInfo.abReasonList = resp.obj.abReasonList
        }
      })
    },
    /**
    ...
    abnormalHandle() {
      this.diaFormInfo.ref.validate(valid => {
        if (valid) {
          this.$store.dispatch(this.store + 'abnormalHandle', this.diaFormInfo.data).then(() => {
            const resp = this.$store.state[this.modName].abnormalHandleResp
            if (resp.code === this.$successCode) {
              this.dialogInfo.visible = false
              this.queryTableData(this.topForm.data)
            }
