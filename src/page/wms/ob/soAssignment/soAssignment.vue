 ...
 * @LastEditTime: 2021-11-02 16:08:54
-->
    ...
    <div slot="tab-body" class="tab-body_auto">
      ...
      >
        <template v-slot:bt-slotEvent="scope">
          <el-button v-if="scope.data.row.assignStatus !='ASSIGN_ALL' " size="mini" type="success" :disabled="$hasPerm('assign')" @click="assignEvent(scope.data.row)">分配</el-button>
          <el-button v-if="scope.data.row.assignStatus =='ASSIGN_PART' || scope.data.row.assignStatus == 'ASSIGN_ALL'" size="mini" type="danger" :disabled="$hasPerm('cancel_assign')" @click="cancelAssignEvent(scope.data.row)">取消分配</el-button>
        </template>
      </zhqc-table>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      checkedIds: [],
      batchBtnArray: [
        { label: this.$t('soAssignment.assign'), type: 'success', icon: '', event: 'handleAssign', btLoading: false, show: !this.$hasPerm('assign'), disabled: true },
        { label: this.$t('soAssignment.cancelAssign'), type: 'danger', icon: '', event: 'handleCancelAssign', btLoading: false, show: !this.$hasPerm('cancelAssign'), disabled: true }
      ],
      dialogInfo: {
        title: '',
        visible: false,
