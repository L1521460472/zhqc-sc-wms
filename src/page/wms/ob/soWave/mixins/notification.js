    ...
    handleCreateWave() {
      ...
      this.diaFormInfoDt.ref.validate(valid => {
        if (valid) {
          ...
          const req = {
            ids: this.checkedIds,
            pickMode: this.diaFormInfoDt.data.pickMode,
            waveOrderLevel: this.diaFormInfoDt.data.waveOrderLevel
          }
          this.$store.dispatch(this.store + 'createWave', req).then(() => {
            const resp = this.$store.state[this.modName].createWaveResp
            if (resp.code === this.$successCode) {
              this.closeDiaDt()
