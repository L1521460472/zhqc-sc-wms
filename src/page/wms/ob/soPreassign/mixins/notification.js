...
export default {
  methods: {
    ...
    handlePreassign() {
      ...
      this.$showLoading()
      this.$store.dispatch(this.store + 'doPreassign', this.checkedIds).then(() => {
        const resp = this.$store.state[this.modName].doPreassignResp
     ...
     * 取消预分配
     */
    handleCancelPreassign() {
      if (!this.checkedIds || this.checkedIds.length == 0) {
        this.$message.warning('请先勾选SO单')
        return
      }
      this.$showLoading()
      this.$store.dispatch(this.store + 'cancelPreassign', this.checkedIds).then(() => {
        const resp = this.$store.state[this.modName].cancelPreassignResp
        if (resp.code === this.$successCode || resp.code === this.$partSuccessCode) {
          this.$message.success(resp.msg)
          this.queryTableData(this.topForm.data)
        }
    ...
    preassignEvent(data) {
      const req = [data.id]
      this.$showLoading()
      this.$store.dispatch(this.store + 'doPreassign', req).then(() => {
        const resp = this.$store.state[this.modName].doPreassignResp
        if (resp.code === this.$successCode || resp.code === this.$partSuccessCode) {
          this.$message.success(resp.msg)
          this.queryTableData(this.topForm.data)
    ...
    cancelPreassignEvent(data) {
      const req = [data.id]
      this.$showLoading()
      this.$store.dispatch(this.store + 'cancelPreassign', req).then(() => {
        const resp = this.$store.state[this.modName].cancelPreassignResp
        if (resp.code === this.$successCode || resp.code === this.$partSuccessCode) {
          this.$message.success(resp.msg)
          this.queryTableData(this.topForm.data)
