...
export default {
  methods: {
    ...
    reboot() {
      ...
      this.topForm.data.ownerShortName = null
      this.topForm.data.ownerType = null
      this.topForm.data.contactName = null
      this.topForm.data.contactPhone = null
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          this.listTypeInfo.ownerTypeList = resp.obj.ownerTypeList
