import { queryTree, initPage, initUpdate, saveData, editData, deleteData } from './../api'
const skuCategory = {
  namespaced: true,
  state: {
    treeObj: null,
  ...
  mutations: {
    ...
    QUERY_TREE(state, data) {
      state.treeObj = data.obj || []
    },
  ...
  actions: {
    // 查询事件
    async queryTree({ commit }, obj) {
      const resp = await queryTree(obj)
      commit('QUERY_TREE', resp)
    },
    // 初始化页面事件
