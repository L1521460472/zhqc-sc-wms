...
export default {
  methods: {
     ...
     */
    reboot() {
      this.topForm.data.containerTypeCode = null
      this.topForm.data.containerTypeName = null
      this.topForm.data.isEnable = null
      this.topForm.data.containerType = null
    },
    /**
     * 翻页
     * @param val
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          ...
          this.listTypeInfo.containerTypeList = resp.obj.containerTypeList
        }
      })
    },
    /**
     ...
     * 带格子选中事件
     */
    isHaveLatticeChecked() {
      const isHaveLattice = this.isHaveLattice
      this.diaFormInfo.fieldList[10].disabled = !isHaveLattice
    }
