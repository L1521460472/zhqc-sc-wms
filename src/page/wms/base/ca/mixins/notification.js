...
export default {
  methods: {
    ...
    reboot() {
      ...
      // this.topForm.data.whAreaName = null
      // this.topForm.data.carrierWhName = null
      ...
      this.diaFormInfoDt.ref.validate(valid => {
        if (valid) {
          for (let i = 0; i < this.diaFormInfo.dtTableInfo.dtList.length; i++) {
            ...
            if (exist.attrCode == this.diaFormInfoDt.data.attrCode) {
              this.$message.error('已存在,保存失败')
              return
            }
          }
          const data = {
            companyCode: this.diaFormInfo.data.companyCode,
            whId: this.diaFormInfo.data.whId,
            attrCode: this.diaFormInfoDt.data.attrCode,
            attrName: this.diaFormInfoDt.data.attrName,
            attrValue: this.diaFormInfoDt.data.attrValue,
            categoryCode: this.diaFormInfoDt.data.categoryCode,
            categoryName: this.diaFormInfoDt.data.categoryName,
            seqNo: this.diaFormInfoDt.data.seqNo,
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          ...
          this.listTypeInfo.sysCarrierList = resp.obj.sysCarrierList
          this.listTypeInfo.origSysList = resp.obj.origSysList
          this.listTypeInfo.pushTypeList = resp.obj.pushTypeList
        }
      })
    },
    /**
    ...
    queryRowData(data) {
      this.$store.dispatch(this.store + 'initUpdate', data.id).then(() => {
        ...
        this.diaFormInfo.data.carrierId = obj.entity.carrierId ? Number(obj.entity.carrierId) : obj.entity.carrierId
        this.diaFormInfo.dtTableInfo.dtList = obj.dtList
        // this.diaFormInfo.pushConfig.data = obj.pushConfig
    ...
    saveData() {
      this.diaFormInfo.ref.validate(valid => {
        if (valid) {
          ...
          // this.diaFormInfo.data.pushConfig = this.diaFormInfo.pushConfig.data
