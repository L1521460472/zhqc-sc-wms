 ...
 * @LastEditTime: 2021-11-02 15:15:19
-->
<template>
  <layout-body>
    <div slot="top-form">
    ...
    <div slot="left-btn">
      ...
      <export-vue
        template-name="maintainUserService"
    ...
    >
      ...
      >
        <template v-slot:form-userNo="scope">
          <list-user :select-key="diaFormInfo.data.userNo" @select="selectUser" />
        </template>
      </zhqc-form>
    </zhqc-dialog>
  </layout-body>
...
import maintainUserMixins from './mixins'
import listUser from '@/Subassembly/ZhqcList/ListUser'

export default {
  name: 'MaintainUser',
  components: { listUser },
  mixins: [maintainUserMixins],
  data() {
    return {
      store: 'maintainUser/',
      modName: 'maintainUser',
      formType: null,
      viewFlag: null,
      ...
      exportUrl: VUE_APP_WMS_MODEL + '/gsp/pro/maintainUser/export'
    }
  },
  computed: {
  ...
  methods: {
    selectUser(data, obj) {
      this.$set(this.diaFormInfo.data, 'userNo', obj.userNo)
      this.diaFormInfo.data.maintainUser = obj.userNo
      this.diaFormInfo.data.maintainUserName = obj.userName
    }
  }
}
</script>
