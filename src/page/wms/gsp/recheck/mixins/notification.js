...
export default {
  methods: {
    ...
    reboot() {
      ...
      this.topForm.data.recheckDateBegin = null
      this.topForm.data.recheckDateEnd = null
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        this.listTypeInfo.recheckStatusList = this.$store.state[this.modName].initPageObj.recheckStatusList
      })
    },
    /**
     * 查询表格数据
