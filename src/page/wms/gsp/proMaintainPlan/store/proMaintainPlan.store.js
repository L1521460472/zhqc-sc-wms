import { pageInfo, initPage, initUpdate, saveData, editData, deleteData, deleteDt, queryMaintainDetermineInStock } from '../api'
...
const proMaintainPlan = {
  ...
  state: {
    ...
    inStocktotal: 0,
    pageResp: null,
    inStockPageResp: null,
    addResp: null,
    editResp: null,
    deleteResp: null,
    initPageObj: null,
    initUpdateObj: null,
    queryMaintainDetermineInStockResp: null,
    deleteDtResp: null
  },
  mutations: {
    ...
    // 查询养护确定表在库产品
    QUERY_MAINTAIN_DETERMINE_IN_STOCK(state, data) {
      if (data.code === state.successCode) {
        message.success(data.msg)
      }
      state.inStocktotal = data.total
      state.inStockPageResp = data.obj || []
    },
    // 删除养护计划明细
  ...
  actions: {
    ...
    // 查询养护确定表在库产品
    async queryMaintainDetermineInStock({ commit }, obj) {
      const resp = await queryMaintainDetermineInStock(obj)
      commit('QUERY_MAINTAIN_DETERMINE_IN_STOCK', resp)
    },
    // 删除养护计划明细
