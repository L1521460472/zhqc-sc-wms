...
export default {
  methods: {
    ...
    reboot() {
      ...
      this.topForm.data.ncrDateBegin = null
      this.topForm.data.ncrDateEnd = null
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        this.listTypeInfo.recheckStatusList = this.$store.state[this.modName].initPageObj.ncrStatusList
      })
    },
    /**
     * 查询表格数据
