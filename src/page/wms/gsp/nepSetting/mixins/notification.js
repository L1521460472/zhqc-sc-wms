...
export default {
  methods: {
    ...
    reboot() {
      ...
      this.topForm.data.shopId = null
      this.topForm.data.startTimeFrom = null
      this.topForm.data.endTimeFrom = null
      this.topForm.data.startTimeTo = null
      ...
      this.topForm.data.origSystem = null
    },
    /**
     * 翻页
     * @param val
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          this.listTypeInfo.origSystemList = resp.obj.origSystemList
        }
      })
    },
    /**
