...
export default {
  methods: {
    ...
    openCopyPage(data) {
      ...
      // 为弹窗表单对应的字段赋值
      this.diaFormInfo.data = this.$deepClone(data)
      this.$set(this.diaFormInfo.data, 'planStartTime', null)
    },
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          this.listTypeInfo.inventoryTypeList = resp.obj.inventoryTypeList
          this.listTypeInfo.inventoryMethodList = resp.obj.inventoryMethodList
          this.listTypeInfo.isEmptyLotList = resp.obj.yesOrNoList
          this.listTypeInfo.isTransitionalLotList = resp.obj.yesOrNoList
        }
      })
    },
    /**
    ...
    /**
         ...
         */
    deleteData(data) {
      this.$confirm(this.$t('确定要删除当前盘点任务以及对应的盘点单吗？？'), {
        type: 'warning',
        center: true
      }).then(() => {
