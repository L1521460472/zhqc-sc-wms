...
export default {
  methods: {
    ...
    reboot() {
      ...
      this.topForm.data.adjNo = null
      this.topForm.data.adjStatus = null
    ...
    openAddPage() {
      ...
      this.diaFormInfo.data.adjStatus = this.listTypeInfo.adjStatusList.length == 0 ? null : this.listTypeInfo.adjStatusList[0].value
    },
    /**
     * 打开编辑页面
     * @param data
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        this.listTypeInfo.adjStatusList = this.$store.state[this.modName].initPageObj.adjStatusList
