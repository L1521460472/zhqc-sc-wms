import { pageInfo, initPage, initUpdate, saveData, editData, deleteData, audit, cancelAudit, cancel, freedCancel, close } from './../api'
...
const replenish = {
  ...
  mutations: {
    ...
    // 解除取消
    FREED_CANCEL(state, data) {
      state.freedCancelResp = data
    },
    // 关闭
    CLOSE(state, data) {
  ...
  actions: {
    ...
    // 解除取消
    async freedCancel({ commit }, obj) {
      const resp = await freedCancel(obj)
      commit('FREED_CANCEL', resp)
    },
    // 关闭
