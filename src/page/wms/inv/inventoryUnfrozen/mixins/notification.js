...
export default {
  methods: {
    ...
    openAddPage() {
      ...
      this.diaFormInfo.data.unfrozenStatus = this.listTypeInfo.frozenStatusList.length === 0 ? null : this.listTypeInfo.frozenStatusList[0].value
    },
    /**
     * 打开编辑页面
     * @param data
    ...
    openDiaInv() {
      ...
      this.dialogInfoInv.title = '冻结单产品参照'
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        console.info(this.$store.state[this.modName].initPageObj)
        this.listTypeInfo.unfrozenStatusList = this.$store.state[this.modName].initPageObj.unfrozenStatusList
        this.listTypeInfo.unfrozenSourceTypeList = this.$store.state[this.modName].initPageObj.unfrozenSourceTypeList
        this.listTypeInfo.unrozenReasonList = this.$store.state[this.modName].initPageObj.unrozenReasonList
      })
    },
    /**
     * 查询表格数据
        ...
        this.validateSubTableInfo().then(valid => {
          ...
          this.diaFormInfo.data.dtList.forEach(item => {
            const temp = {
              ...
              unfrozenQty: item.unfrozenQty,
              skuCode: item.skuCode,
              skuId: item.skuId,
              origDtId: item.origDtId,
              origLineNum: item.origLineNum,
          ...
          const obj = {
            unfrozenReason: this.diaFormInfo.data.unfrozenReason,
    ...
    batchAuditAdj() {
      let ids = []
      if (!this.idsList || this.idsList.length === 0) {
