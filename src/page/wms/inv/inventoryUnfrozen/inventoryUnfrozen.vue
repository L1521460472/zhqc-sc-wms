  ...
  <layout-body>
    <div slot="top-form">
      ...
      >
        ...
        <!--<list-wh-zone @select="selectZone" :selectKey="topForm.data.zoneId" ></list-wh-zone>-->
        <!--</template>-->
        <!--库区-->
        <template v-slot:form-zoneId="scope">
          <remote-list
    ...
    <div slot="left-btn">
      ...
      <!--      <export-vue templateName="inventoryUnfrozenService"-->
      <!--                  exportUrl="wms/inv/inventoryFrozen/export"-->
      <!--                  exportName="导出库内管理-库存冻结单"-->
    ...
    <full-pop
      ...
      @handleClick="handleClick"
    >
      <full-pop-item full-pop-item-title="库存解冻单">
        <zhqc-form
...
import inventoryUnfrozenMixins from './mixins'
...
export default {
  name: 'InventoryUnfrozen',
  components: {
    zhqcListOwner, buttonList, listWhZone, listWhLot
  },
  mixins: [inventoryUnfrozenMixins],
  data() {
    return {
      store: 'inventoryUnfrozen/',
      modName: 'inventoryUnfrozen',
      collapsable: false, // 展开收缩
      pageRequest: { limit: this.$globalLimit, page: this.$globalPage },
      pageInStockProRequest: { limit: this.$globalLimit, page: this.$globalPage },
      ownerUrl: process.env.VUE_APP_WMS_MODEL + '/base/partner/owner/queryOwnerCbList',
      zoneUrl: process.env.VUE_APP_WMS_MODEL + '/base/wh/whZone/queryWhZoneCbList',
      lotUrl: process.env.VUE_APP_WMS_MODEL + '/base/wh/whLot/queryWhLotCbList',
      salesStorehouseUrl: process.env.VUE_APP_WMS_MODEL + '/base/wh/whArea/queryVirtualwhList',
      formType: null,
      viewFlag: null,
      dialogInfo: {
        title: '',
