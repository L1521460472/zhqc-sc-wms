...
export default {
  methods: {
     ...
     */
    reboot() {
      this.topForm.data.moveLotOrderNo = null
      this.topForm.data.origin = null
      this.topForm.data.moveLotOrderStatus = null
      this.topForm.data.ownerId = null
      this.topForm.data.creator = null
      this.topForm.data.moveTimeFrom = null
      this.topForm.data.moveTimeTo = null
     ...
     */
    openDiaInv() {
      const selectFmZoneId = this.diaFormInfo.data.fmZoneId
      const selectToZoneId = this.diaFormInfo.data.toZoneId
      const selectOwnerId = this.diaFormInfo.data.ownerId
      if (this.$isEmpty(selectFmZoneId)) {
        this.$message.error('请选择来源库区')
        return
      }
      if (this.$isEmpty(selectToZoneId)) {
        this.$message.error('请选择来目标库区')
        return
      }
      if (this.$isEmpty(selectOwnerId)) {
      ...
      this.dialogInfoInv.topFormInv.data.fmZoneId = selectFmZoneId
      this.dialogInfoInv.topFormInv.data.ownerId = selectOwnerId
      this.initDataQueryInStockProTableData()

      const that = this
      setTimeout(function() {
        that.fmZoneId = selectFmZoneId
        that.toZoneId = selectToZoneId
      }, 500)
    },
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        if (resp.code === this.$store.state[this.modName].successCode) {
          this.listTypeInfo.moveLotOrderStatusList = resp.obj.moveLotOrderStatusList
          this.listTypeInfo.moveLotOrderOriginList = resp.obj.moveLotOrderOriginList
        }
      })
    },
    /**
    ...
    queryInStockProTableData(data) {
      this.$showLoading()
      this.$store.dispatch(this.store + 'queryCanMoveStock', data).then(() => {
      }).finally(() => {
        this.$hideLoading()
      })
    },
    ...
    closeInStockPro() {
      ...
      this.dialogInfoInv.topFormInv.data.fmLotCode = null
      this.dialogInfoInv.topFormInv.data.toLotCode = null
    ...
    determine() {
      ...
      if (this.inStockProList.length == 0) {
        ...
        for (let i = 0; i < tempInStockArr.length; i++) {
          tempInStockArr[i].fmZoneId = this.diaFormInfo.data.fmZoneId// 设置来源库区
          tempInStockArr[i].toZoneId = this.diaFormInfo.data.toZoneId// 设置目的库区
          tempInStockArr[i].toLotCode = this.dialogInfoInv.topFormInv.data.toLotCode// 设置目的库位
        }
        ...
        for (let i = 0; i < tempDistinctList.length; i++) {
          if (distinctList.indexOf(tempDistinctList[i]) === -1) {
            tempInStockProList[i].fmZoneId = this.diaFormInfo.data.fmZoneId// 设置来源库区
            tempInStockProList[i].toZoneId = this.diaFormInfo.data.toZoneId// 设置目的库区
            tempInStockProList[i].toLotCode = this.dialogInfoInv.topFormInv.data.toLotCode// 设置目的库位
      ...
      this.closeInStockPro()// 关闭页面在库产品在库参照页面
    },
