...
export default {
  methods: {
    ...
    */
    countInfo() {
      this.$store.dispatch(this.store + 'countInfo', this.topForm.data).then(() => {
        const resp = this.$store.state[this.modName].countInfoResp
        if (resp.code === this.$successCode) {
          this.consumeSkuTotalQty = resp?.obj?.consumeSkuTotalQty ?? ''
        }
      })
    },
