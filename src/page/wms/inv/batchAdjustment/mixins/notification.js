...
export default {
  methods: {
    ...
    openDiaInv() {
      ...
      const selectedOwnerCode = this.diaFormInfo.data.ownerCode
      const selectedWhAreaId = this.diaFormInfo.data.whAreaId
      // const ownerId = this.ownerId
      if (this.$isEmpty(selectedOwnerCode)) {
      ...
      this.dialogInfoInv.topFormInv.data.ownerCode = selectedOwnerCode
    ...
    initPage() {
      this.$store.dispatch(this.store + 'initPage', {}).then(() => {
        ...
        this.listTypeInfo.skuStatusList = result?.skuStatusList ?? []
      })
    },
    /**
     * 查询表格数据
     ...
     */
    editData() {
      const obj = this.diaFormInfo.data
      console.log(obj)
      this.diaFormInfo.ref.validate(valid => {
        if (valid) {
    ...
    determine_add() {
      ...
      this.diaFormInfo.data.dtList = this.diaFormInfo.data.dtList.map(item => {
        return { ...item, tProductionBatch: null, tProductionDate: null, adjQty: null, tLotCode: null }
      })
    ...
    determine_update() {
      ...
      if (this.diaFormInfo.data.dtList === 0) {
        ...
        this.tempInStockProList.forEach((item) => {
          ...
          item.tProductionBatch = ''
          item.tProductionDate = ''
          item.adjQty = ''
          item.tLotCode = ''
        })
    ...
    focusEvent(data) {
      ...
      const params = {
        ...
        'batchNo': 'str',
        'paType': data.row.lotQuality == 'BHGP' ? 'SHBLPSJ' : 'SHLPSJ',
