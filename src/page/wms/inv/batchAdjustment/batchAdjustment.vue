<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
  <layout-body>
    ...
    <full-pop
      ...
      @handleClick="handleClick"
    >
      <full-pop-item full-pop-item-title="批次调整单">
        <zhqc-form
      ...
      <full-pop-item full-pop-item-title="商品明细">
        <vex-dia-table
        ...
        >
          ...
          <template v-slot:col-tLotCode="scope">
            <ListAutocomplete
              ...
              :item="{label:$t('batchAdjustment.dt.targetLotCode')}"
              :model="scope.row"
            />
          </template>
        </vex-dia-table>
...
export default {
  ...
  data() {
    return {
      store: 'batchAdjustment/',
      modName: 'batchAdjustment',
      collapsable: false, // 展开收缩
      pageRequest: { limit: this.$globalLimit, page: this.$globalPage },
      pageInStockProRequest: { limit: this.$globalLimit, page: this.$globalPage },
      formType: null,
      viewFlag: null,
      exportUrl: process.env.VUE_APP_WMS_MODEL + '/pd/batchAdjustment/export',
  ...
  computed: {
    ...
    InStockProPageResp() {
      const arr = this.$store.state[this.modName].InStockProPageResp || []
      //  arr.forEach(item => {
      //    item.productionDate=
