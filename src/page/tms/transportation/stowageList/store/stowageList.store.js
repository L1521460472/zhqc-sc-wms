import { pageInfo, initPage, initUpdate, saveData, editData, deleteData, affirmDepart, cancel, quickInCar, quickOutCar } from './../api'
// import { Message } from 'element-ui'
const stowageList = {
  namespaced: true,
  state: {
    total: 0,
    pageResp: null,
    addResp: null,
    editResp: null,
    deleteResp: null,
    initPageObj: null,
    initUpdateObj: null,
    affirmResp: null,
    cancelResp: null,
    quickInResp: null,
    quickOutResp: null
  },
  mutations: {

    PAGE_INFO(state, data) {
      state.total = data.total
      state.pageResp = data.obj || []
    },
    INIT_PAGE(state, data) {
      state.initPageObj = data.obj || {}
    },
    INIT_UPDATE(state, data) {
      state.initUpdateObj = data.obj || {}
    },
    SAVE_DATA(state, data) {
      state.addResp = data
    },
    EDIT_DATA(state, data) {
      state.editResp = data
    },
    DELETE_DATA(state, data) {
      state.deleteResp = data
    },
    AFFIRM_DATA(state, data) {
      state.affirmResp = data
    },
    CANCEL_DATA(state, data) {
      state.cancelResp = data
    },
    QUICK_IN_DATA(state, data) {
      state.quickInResp = data
    },
    QUICK_OUT_DATA(state, data) {
      state.quickOutResp = data
    }
  },
  actions: {
    // 查询事件
    async pageInfo({ commit }, obj) {
      const resp = await pageInfo(obj)
      commit('PAGE_INFO', resp)
    },
    // 初始化页面事件
    async initPage({ commit }, obj) {
      const resp = await initPage(obj)
      commit('INIT_PAGE', resp)
    },
    // 初始化更新对象事件
    async initUpdate({ commit }, obj) {
      const resp = await initUpdate(obj)
      commit('INIT_UPDATE', resp)
    },
    // 新增保存事件
    async saveData({ commit }, obj) {
      const resp = await saveData(obj)
      commit('SAVE_DATA', resp)
    },
    // 修改保存事件
    async editData({ commit }, obj) {
      const resp = await editData(obj)
      commit('EDIT_DATA', resp)
    },
    // 删除数据事件
    async deleteData({ commit }, obj) {
      const resp = await deleteData(obj)
      commit('DELETE_DATA', resp)
    },
    async affirmDepart({ commit }, obj) {
      const resp = await affirmDepart(obj)
      commit('AFFIRM_DATA', resp)
    },
    async cancel({ commit }, obj) {
      const resp = await cancel(obj)
      commit('CANCEL_DATA', resp)
    },
    async quickInCar({ commit }, obj) {
      const resp = await quickInCar(obj)
      commit('QUICK_IN_DATA', resp)
    },
    async quickOutCar({ commit }, obj) {
      const resp = await quickOutCar(obj)
      commit('QUICK_OUT_DATA', resp)
    }
  }
}
export default stowageList
