 ...
 * @LastEditTime: 2021-11-02 10:23:30
-->
  ...
  <layout-body>
    ...
    >
      <zhqc-form
        ...
        @handleEvent="handleEvent"
      >
        <template v-slot:form-cityCode="">
          <zhqc-list-town :select-key="diaFormInfo.data.cityCode" :province-id="provinceId" :disabled="dialogInfo.type=='view'" @select="selectCity" />
        </template>
      </zhqc-form>
    </zhqc-dialog>
  </layout-body>
...
<script>
import countyMixins from './mixins'
import zhqcListTown from '@/Subassembly/ZhqcList/ListTown/ListTown'
...
export default {
  ...
  methods: {
    selectProvince(data) {
      this.$set(this.diaFormInfo.data, 'provinceId', data)
      this.diaFormInfo.data.cityCode = null
      this.provinceId = data
      this.cityCode = null
    },
    selectCity(data) {
      this.$set(this.diaFormInfo.data, 'cityCode', data)
      this.cityCode = data
    }
  }
}
