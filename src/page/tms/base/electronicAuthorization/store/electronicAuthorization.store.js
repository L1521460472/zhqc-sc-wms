import { pageInfo, initPage, initUpdate, saveData, editData, deleteData, queryOwnerList, deactivate, enable, queryCustomWh } from './../api'
// import { Message } from 'element-ui'

const electronicAuthorization = {
  namespaced: true,
  state: {
    total: 0,
    pageResp: null,
  ...
  mutations: {
    ...
    CUSTOMWH_LIST(state, data) {
      state.customListResp = data
    },
  ...
  actions: {
    ...
    async queryCustomWh({ commit }) {
      const resp = await queryCustomWh()
      commit('CUSTOMWH_LIST', resp)
    },
...
export default electronicAuthorization
