import { pageInfo, initPage, initUpdate, saveData, editData, deleteData, enableData, unableData } from './../api'
const preSchedulingStrategy = {
  ...
  mutations: {
    ...
    UNABLE_DATA(state, data) {
      state.unableResp = data
    }
  },
  actions: {
    // 查询事件
    ...
    async enableData({ commit }, obj) {
      const resp = await enableData(obj)
      commit('ENABLE_DATA', resp)
    },
    async unableData({ commit }, obj) {
      const resp = await unableData(obj)
      commit('UNABLE_DATA', resp)
    }
  }
}
