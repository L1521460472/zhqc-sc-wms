...
export default {
  ...
  data() {
    return {
      ...
      list: [],
      loading: false,
      // listUrl: process.env.VUE_APP_WMS_MODEL + '/ob/waveModel/queryCbList',
      // itemUrl: process.env.VUE_APP_WMS_MODEL + '/ob/waveModel/queryCb/'
      listUrl: VUE_APP_WMS_MODEL + '/ob/waveModel/queryCbList',
      itemUrl: VUE_APP_WMS_MODEL + '/ob/waveModel/queryCb/'
    }
  },
  computed: {},
  watch: {
  ...
  methods: {
    ...
    /**
       ...
       */
    listApi(queryText) {
      const data = { modelName: queryText, page: this.$globalPage, limit: this.$globalLimit }
      return service({
        url: this.listUrl,
        method: 'post',
        data
