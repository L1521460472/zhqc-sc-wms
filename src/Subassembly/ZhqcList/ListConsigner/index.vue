...
export default {
  ...
  data() {
    return {
      ...
      list: [],
      loading: false,
      // listUrl: process.env.WMS_MODEL+'/base/wh/wh/selectwhInfo',
      // listUrl: process.env.WMS_MODEL+'/base/wh/wh/whName',
      // listUrl: process.env.VUE_APP_WMS_MODEL
      listUrl: VUE_APP_WMS_MODEL
    }
  },
  computed: {},
  watch: {
  ...
  methods: {
    ...
    /**
       ...
       */
    listApi(queryText) {
      const data = { keyWord: queryText, page: this.$globalPage, limit: this.$globalLimit }
      let reqUrl
      if (!this.inOrderUrl) {
        reqUrl = this.listUrl + '/base/wh/wh/whName'
      } else {
        reqUrl = this.listUrl + this.inOrderUrl
      }
