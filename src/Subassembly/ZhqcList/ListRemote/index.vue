...
export default {
  ...
  methods: {
    ...
    /**
             ...
             * @param queryText 查询文本
             *
             * @param requestStyle  1 VUE_APP_WMS_MODEL + '/base/partner/owner/queryOwnerCbList',
             *                       2  VUE_APP_WMS_MODEL + '/base/sku/sku/querySkuCbList'
             *                       3
             */
    listApi(queryText) {
      let data = {}
      if (this.requestStyle === 1) {
        // type  0:公司级别,1:实体仓级别 销售仓
        data = { keyWord: queryText, type: this.type }
      } else if (this.requestStyle === 2) {
        data = { 'ownerId': this.ownerId, 'page': 1, 'limit': 50, 'queryText': queryText, 'isConsumables': 1 }
      } else if (this.requestStyle === 3) {
        data = {}
      }
      return service({
        url: this.listUrl,
      ...
      if (this.list && value) {
        for (let i = 0; i < this.list.length; i++) {
          ...
          if (rowvalue === value) {
            ...
            if (this.optionsProps.link) this.model[item.link] = row[this.optionsProps.link]
            this.$emit('select', value, row, this.model)
          }
        }
      } else {
