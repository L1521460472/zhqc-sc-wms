          ...
          <div :class="[index == hoverIndex ? 'image-item' : 'image-item-none']">
            <div class="icon-ju-wei">
              <div style="width: 100%">
                ...
                <span v-if="!disabled" class="el-icon-download icon-margin" @click="downloadItem(item)" />
  ...
  methods: {
    initOptions() {
      if (Object.prototype.toString.call(this.accept) === '[object String]') {
        ...
        const accept = this.accept || [{ type: 'pdf', limit: 2 }, { type: 'img', limit: 3 }]
        this.uploadOp.accept = accept.map(item => {
          ...
          if (item.type === 'img') {
            ...
            this.uploadOp.limit += item.limit
            return '.jpg,.png,.JPG,.PNG,.bmp,.BMP'
          }
        }).toString()
    ...
    beforeAvatarUpload(file) {
      ...
      const isType = (isJPG || isPNG || isPdf || isDoc || isBmp) // 限制图片类型
      const isLimit = file.size / 1024 / 1024 < this.size // 限制图片大小
      if (!isType) {
        this.$message.error(`（只允许上传${this.uploadOp.limitPdf ? 'PDF，' : ''}${this.uploadOp.limitDoc ? '文档，' : ''}BMP、JPG 或 PNG 格式图片！`)
      }
      if (!isLimit) {
    ...
    downloadItem(imgpath) {
      const imgsrc = decodeURI(imgpath)
    ...
    /* 文件上传前校验 */
    validate(file) {
      if (/.pdf/g.test(file.name) && this.imageView.previewSrcList.filter(item => item.endsWith('.pdf') || item.endsWith('.PDF')).length === this.uploadOp.limitPdf) {
      ...
      if (/.jpg|.png|.bmp|.JPG|.PNG|.BMP/g.test(file.name) && this.imageView.previewSrcList.filter(item => item.endsWith('.jpg') || item.endsWith('.JPG') || item.endsWith('.png') || item.endsWith('.PNG') || item.endsWith('.bmp') || item.endsWith('.BMP')).length === this.uploadOp.limitImg) {
